                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (!tx.CheckInputs(view, CS_ALWAYS, false, false))
        {
...
bool CWalletTx::AcceptWalletTransaction(bool fCheckInputs)
{

    {
        ...
        BOOST_FOREACH(CMerkleTx& tx, vtxPrev)
        {
            ...
            {
                uint256 hash = tx.GetHash();
                if (!mempool.exists(hash) && pcoinsTip->HaveCoins(hash)) // TODO: Originally was NOT contains Coins
...
bool CTransaction::CheckInputs(CCoinsViewCache &inputs, enum CheckSig_mode csmode, bool fStrictPayToScriptHash, bool fStrictEncodings, CBlock *block) const
{
    if (!IsCoinBase())
    {
        ...
        if (csmode == CS_ALWAYS ||
            ...
            for (unsigned int i = 0; i < vin.size(); i++) {
                ...
                if (!VerifySignature(coins, *this, i/*, fStrictPayToScriptHash*/, fStrictEncodings, 0))
                {
                    ...
                    if (fStrictPayToScriptHash && VerifySignature(coins, *this, i, fStrictEncodings, 0))
        ...
        else
        {
            ...
            if (nTxFee < GetMinFee())
                return (csmode==CS_AFTER_CHECKPOINT)? DoS(100, error("ConnectInputs() : %s not paying required fee=%s, paid=%s", GetHash().ToString().substr(0,10).c_str(), FormatMoney(GetMinFee()).c_str(), FormatMoney(nTxFee).c_str())) : false;
...
bool CTransaction::ClientCheckInputs() const
{
    ...
    // Take over previous transactions' spent pointers
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Verify signature
            if (!VerifySignature(CCoins(txPrev, -1), *this, i, true, 0))
...
bool CBlock::DisconnectBlock(CBlockIndex* pindex, CCoinsViewCache &view)
{
    ...
    for(unsigned int i=0; i<vtx.size(); i++)
    {
        ...
        SyncWithWallets(GetTxHash(i),tx, this, false, false);
    }
...
bool SetBestChain(CBlockIndex* pindexNew)
{
    ...
    // List of what to connect (typically nothing)
...
bool LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        if (!CheckStakeModifierCheckpoints(pindex->nHeight, pindex->nStakeModifierChecksum))
            return error("LoadBlockIndex() : Failed stake modifier checkpoint height=%d, modifier=0x%016"PRI64x, pindex->nHeight, pindex->nStakeModifier);
    }
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    if (tx.AcceptToMemoryPool(true, &fMissingInputs2))
                    {
                        ...
                        SyncWithWallets(inv.hash, tx, NULL, true); //TODO: remove
                        RelayMessage(inv, vMsg);
                        mapAlreadyAskedFor.erase(inv);
                        vWorkQueue.push_back(inv.hash);
                        vEraseQueue.push_back(inv.hash);
